	...
	Context("Checks service across nodes", func() {
		...
		SkipItIf(func() bool {
			...
			BeforeAll(func() {
				...
				httpBackends := ciliumIPv6Backends("-l k8s:zgroup=testDS,k8s:io.kubernetes.pod.namespace=default", "80")
				ciliumAddService(31080, net.JoinHostPort(testDSIPv6, string(data.Spec.Ports[0].NodePort)), httpBackends, "NodePort", "Cluster")
				tftpBackends := ciliumIPv6Backends("-l k8s:zgroup=testDS,k8s:io.kubernetes.pod.namespace=default", "69")
				ciliumAddService(31069, net.JoinHostPort(testDSIPv6, string(data.Spec.Ports[1].NodePort)), tftpBackends, "NodePort", "Cluster")
			})
